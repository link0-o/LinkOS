...
; 入口跳转（跳过 GDT 数据区）
;-------------------------------------------------------------------------------
jmp near loader_start                       ; 近跳转 (3字节: E9 xx xx)
nop                                         ; 填充对齐 (1字节)
nop                                     
nop                                      
nop                                      
nop                                         
; 现在正好 8 字节，GDT_BASE 从 0x908 开始（8字节对齐）

;-------------------------------------------------------------------------------
; 全局描述符表 (GDT - Global Descriptor Table)
;-------------------------------------------------------------------------------
; GDT 从 0x908 开始，8字节对齐
...
; GDT 相关宏定义
;-------------------------------------------------------------------------------
GDT_DESC_CNT equ 4                          ; 当前已定义的描述符数量
GDT_SIZE    equ $ - GDT_BASE                ; 已定义 GDT 大小（字节）
GDT_LIMIT   equ GDT_SIZE - 1                ; 仅已定义部分的界限

times 60 dq 0    ; 预留 60 个描述符空位（60 × 8 = 480 字节）

; 包含预留空间的总 GDT 界限：(4 + 60) * 8 - 1 = 511
GDT_TOTAL_LIMIT equ (GDT_DESC_CNT + 60) * 8 - 1

;-------------------------------------------------------------------------------
; 内存检测相关变量
;-------------------------------------------------------------------------------
; total_mem_bytes 的内存地址计算：
;   GDT_BASE (0x908) + 4×8 (已定义的描述符) + 60×8 (预留空位) = 0xB08
...
; GDT 指针结构（6 字节）：
;   前 2 字节：GDT 界限（使用包含预留空间的总界限）
;   后 4 字节：GDT 基地址
gdt_ptr dw GDT_TOTAL_LIMIT
        dd GDT_BASE
...
; 负责映射虚拟地址 0x00000000 ~ 0x003FFFFF（即前 4MB 的用户空间）
    ...
    mov [PAGE_DIR_TABLE_POS + 0xc00], eax                   ; 临时方案,后面会再进行重新映射
