...
/* 每进程最大打开文件数 (Linux 默认 1024, 因 PCB 与内核栈共享 4KB 页面, 取 128) */
#define MAX_FILES_OPEN_PER_PROC 128
...
struct task_struct{
    ...
    int32_t fd_table[MAX_FILES_OPEN_PER_PROC]; // 文件描述符表（进程局部 fd → 全局文件表下标）
    uint32_t cwd_inode_nr;          // 当前工作目录的 inode 号
