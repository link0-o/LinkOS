...
cls_screen:
    ...
    ; ---- 用 gs 指向显存 ----
    mov ax, 0x18            ; SELECTOR_K_DATA (视频段)... 实际上 gs 在启动时已设好
                            ; 这里不改 gs, 直接用它(已指向显存)

    ; 清空 80*25 = 2000 个字符  (每个字符 2 字节: 字符+属性)
    mov ecx, 2000
    mov edi, 0              ; 从显存偏移 0 开始
.cls_loop:
    mov word [gs:edi], 0x0720  ; 0x07=灰白前景/黑背景, 0x20=' '
    add edi, 2
    loop .cls_loop
